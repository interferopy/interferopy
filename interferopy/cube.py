...
class Cube:
    ...
    def findclumps_1kernel(self, output_file, rms_region=1. / 4., minwidth=3, sextractor_param_file='default.sex',
        ...
        assert not (isfile(output_file + '_kw' + str(int(minwidth)) + '.cat')), 'Output file already exists - please delete or change name!'
        if minwidth % 2 == 1:
        ...
        for k in range(chnbox, nax1 - chnbox - 1):
            ...
            # run Sextractor
            os.system('sex ./tmp_findclumps/' + name_mask_tmp +'.fits' + ' -c ' + sextractor_param_file + ' -CATALOG_NAME ./tmp_findclumps/target_' + name_mask_tmp + '.list  -VERBOSE_TYPE QUIET')
            if clean_tmp:
    ...
    def findclumps_full(self, output_file, kernels=np.arange(3, 20, 2), rms_region=1. / 4.,
                        ...
                        delta_offset_arcsec=2, delta_freq=0.1, ncore=1):
        '''
        ...
        tools.run_line_stats_sex(sextractor_catalogue_name=output_file + '_clumpsP', binning_array=kernels,
                                 SNR_min=min_SNR)
        tools.run_line_stats_sex(sextractor_catalogue_name=output_file + '_clumpsN', binning_array=kernels,
                                 SNR_min=min_SNR)
